...
const translations = {
  ...
  unknown: "לא ידוע - כדאי לבדוק באתר פיקוד העורף",
};
...
const setAlertChannelCommand = async (interaction) => {
  ...
  const channelPermissions = interaction.guild.channels.cache
    .get(channelId)
  ...
  try {
    if (!channelPermissions.has("SendMessages")) {
      interaction.reply({
        content: `Failed to set the alert channel. No permissions`,
        ephemeral: true,
      });
    } else {
    ...
    interaction.reply({
      content: "Failed to set the alert channel. Please try again.",
      ephemeral: true,
    });
  }
};
